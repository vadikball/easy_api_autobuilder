...
from easy_api_autobuilder.v1.constants.constants import (
...
from easy_api_autobuilder.v1.logger import LoggerMixin
from easy_api_autobuilder.v1.schema import BaseModel, BaseParams, FilterOperationEnum

if TYPE_CHECKING:
    from loguru import Logger

schema_cache: dict[str, type[BaseModel]] = {}
schema_factory_cache: dict[str, "SchemaFactory"] = {}  # {Model.__tablename__: Model}
...
class SchemaFactory(LoggerMixin):
    def __new__(cls, model: type[DeclarativeMetaProtocol], logger: "Logger") -> "SchemaFactory":
    ...
    def __init__(self, model: type[DeclarativeMetaProtocol], logger: "Logger"):
        if model.__tablename__ in schema_factory_cache:
            return
    ...
    def create_params_from_model(
        self,
        primary_schema: type[BaseParams] | None = None,
        ...
        if order_fields:
            ...
            self.logger.debug(str(default_enum))
        ...
        self.logger.debug(f"{self._model.__tablename__}, {schema_name}\n{schema_annotations}\n\n")
    ...
    def _resolve_nested(self, field_property: Relationship):
        ...
        nested_schema_factory = SchemaFactory(sub_model, self.logger)
