...
class BaseParams(BaseModel):
    @staticmethod
    def annotate_arg(field_name: str, field: FieldInfo, func_globals: dict, logger: "Logger") -> str:
        default_arg = "None"
        ...
        if alias_marker:
            logger.debug(f"{field.annotation} {field.annotation.__args__}")
        ...
        anno_deps = "Query()" if not base_param_marker else "Depends({0}.params_deps(logger))".format(type_str)
        if anno_deps != "Query()":
    ...
    @classmethod
    def params_deps(cls, logger: "Logger") -> Callable:
        local_ns = {}
        ...
        for field_name, field in cls.model_fields.items():
            func_args.append(cls.annotate_arg(field_name, field, func_globals, logger))
