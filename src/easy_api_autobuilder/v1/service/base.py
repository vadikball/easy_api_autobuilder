...
from easy_api_autobuilder.v1.schema import BaseModel

if TYPE_CHECKING:
    from loguru import Logger


class BaseRepoService(LoggerMixin):
    def __init__(self, repo: BaseRepo, logger: "Logger"):
...
class ListService(BaseRepoService):
            ...
            if field_name not in allow_none:
                if isinstance(field_value, dict):
                    if not field_value.get(NESTED_PARAM_TEMPLATE.format(field_name, FIELD_VALUE_NAME)):
                        self.logger.debug(f"{field_name} dissallow")
                        continue
                else:
    ...
    async def list(self, request_params: PageParams | None = None) -> Page:
        ...
        self.logger.debug(f"{request_params} {filters} {order_by} {order_direction}")
...
class PostService(BaseRepoService):
    ...
    async def post(self, body: BaseModel) -> BaseModel:
        if not isinstance(body, self._input_create):
            raise ValueError(f"service {self.__class__.__name__} can't recognize {body.__class__.__name__} schema")
...
class PutService(BaseRepoService):
    ...
    async def put(self, body: BaseModel, *, model_pk: Any) -> None:
        if not isinstance(body, self._input_update):
            raise ValueError(f"service {self.__class__.__name__} can't recognize {body.__class__.__name__} schema")
