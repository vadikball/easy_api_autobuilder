...
from easy_api_autobuilder.v1.schema import FilterOperationEnum

if TYPE_CHECKING:
    from loguru import Logger


class BaseRepo(LoggerMixin):
    """Base repo for models."""

    _cls_model: type[DeclarativeMetaProtocol]
    ...
    async def get_by_page(
        ...
        self.logger.debug(str(filters))
        filters_exp = self._eval_filters(filters)
        self.logger.debug(str(filters_exp))
        ...
        self.logger.debug(str(count_query))
...
class SecondaryBaseRepo(LoggerMixin):
    ...
    async def delete(self, *, pkey_val: Any, secondary_pkey_val: Any) -> None:
        ...
        self.logger.debug(str(first_primary_key))
        self.logger.debug(str(secondary_primary_key))
